...
#: making offline exhaustive key search feasible (trivially so at K=1,
#: H_inf ~= 19 bits). Enforced as a *warning* only (not a hard error) so
#: that existing small-K KAT/test vectors keep working; see
#: :func:`generate_prime_numbers` and :func:`_validate_key`.
MIN_KEY_COUNT = 7
...
def _warn_if_weak_key_count(count: int) -> None:
    ...
    A key with K < 7 prime elements has H_inf(key) < 128 bits and is not
    conformant with the IND-CPA security claim of
    ``docs/napseq-eprint-preprint.tex`` (Theorem IND-CPA, Remark ``min-K``).
    It remains fully *usable* — this is intentionally a warning, not a
    ``ValueError`` — since the existing KAT corpus includes deliberately
    small (e.g. K=1) keys for coverage/negative testing.
    """
    if count < MIN_KEY_COUNT:
        warnings.warn(
            f"Key element count K={count} is below the recommended minimum "
            f"of {MIN_KEY_COUNT}. H_inf(key) < 128 bits at this size, making "
            "offline exhaustive key search feasible (see "
...
def _compute_auth_tag(kb: bytes, aad: bytes, payload: bytes) -> bytes:
    ...
    with ``ctx = be4(len(aad)) || aad || masked_blob``.  ``payload`` is
...
def generate_v8_key(count: int = 10,
                    min_val: int = 1_000_000,
                    max_val: int = 15_000_000) -> tuple[list[int], bytes]:
...
def _synthetic_nonce(sk_fmt: bytes, aad: bytes, message: bytes) -> bytes:
    ...
    ``N = HMAC(sk_fmt, 0x0A || be4(len(aad)) || aad || message)[0:16]``.
...
def _decrypt_v8_core(nonce: bytes, cypher: list[int], key: list[int],
    ...
    while real_idx < real_count:
        ...
        k = key[real_idx % K]
        addend = _derive_addend(key_material, nonce, real_idx, k)
        padded.append((cypher[ct_pos] - addend) // k)
        ct_pos += 1
        real_idx += 1
...
def encrypt_bytes_v8(message: str, primes: list[int], sk: bytes,
    ...
    **independently**-sampled 256-bit subkey (see :func:`generate_v8_key`)
    ...
    tokens = _encrypt_v8_core(codepoints, primes, sk_fmt, nonce)
    token_blob = _fixed_encode_tokens(tokens)
    ks = _varint_keystream(sk_fmt, nonce, len(token_blob))
    masked_blob = bytes(a ^ b for a, b in zip(token_blob, ks))
    payload = nonce + masked_blob
    tag = _compute_auth_tag(sk_fmt, aad, payload)
    return payload + tag
...
def decrypt_bytes_v8(ciphertext: bytes, primes: list[int], sk: bytes,
    ...
    payload = ciphertext[:-_TAG_SIZE]
    recv_tag = ciphertext[-_TAG_SIZE:]
    calc_tag = _compute_auth_tag(sk_fmt, aad, payload)
...
def encrypt_str_v8(message: str, primes: list[int], sk: bytes,
    ...
    return base64.b64encode(encrypt_bytes_v8(message, primes, sk, aad)).decode("ascii")
