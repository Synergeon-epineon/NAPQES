...
prime-indexed token cipher, and a uniform 256-bit secret $\mathsf{sk}$.
From $\mathsf{sk}$, a per-wire-format subkey
$\mathsf{sk}_{\mathrm{fmt}}=\mathrm{HMAC}(\mathsf{sk},
\texttt{0x0B}\|\mathrm{format\_id})$ is derived once (domain
$\texttt{0x0B}$; format $\texttt{0x01}$ for the block mode specified in
this paper, Section~\ref{sec:domains}), and every subsequent HMAC domain
derivation used for noise injection, plaintext padding, keystream
masking, message authentication, and synthetic-nonce derivation is keyed
by $\mathsf{sk}_{\mathrm{fmt}}$. This layer binds every derivation to
one wire format so that a tag produced under one format cannot verify
under another sharing the same $(\mathbf{k},\mathsf{sk})$. The nonce is
derived synthetically as
$N=\mathrm{HMAC}(\mathsf{sk}_{\mathrm{fmt}},\texttt{0x0A}\|\mathrm{be8}(|A|)\|A\|M)[0{:}16]$,
in
the manner of AES-GCM-SIV, rather than sampled independently at random.
Because $\mathbf{k}$ and $\mathsf{sk}$ are sampled independently and the
only HMAC calls in the construction are keyed either by $\mathsf{sk}$
(the single $\texttt{0x0B}$ format-subkey call) or by the
$\mathsf{sk}$-derived $\mathsf{sk}_{\mathrm{fmt}}$ (every other domain),
we prove IND-CPA-det (the
...
\begin{enumerate}
  \item We fully specify NAPQES (Section~\ref{sec:construction}): an
        ...
        $N=\mathrm{HMAC}(\mathsf{sk}_{\mathrm{fmt}},\texttt{0x0A}\|\mathrm{be8}(|A|)\|A\|M)$
...
\begin{itemize}
  ...
  \item $[0{:}n]_{\mathrm{be}k}$ denotes the first $n$ bytes of a 32-byte
        ...
        in $[0,2^{k})$ where $k=8n$.
  \item $\mathcal{P}$ denotes the set of prime integers in $[10^6, 1.5 \times 10^7)$,
        a fixed public system parameter with $|\mathcal{P}| = 892{,}206$
        (verified by sieve; the interval is half-open, equivalently the
        inclusive interval $[10^6, 14{,}999{,}999]$, and both endpoints are
        composite). Implementations MUST use this interval:
        a different upper bound yields a different $\mathcal{P}$, a
        different canonical enumeration, and hence different keys.
        This is the same prime set from which the key-establishment layer
        (Section~\ref{sec:keygen}) draws, so the AEAD and the KEM share one
        parameter set.
\end{itemize}
...
cryptographically secure DRBG (default $K=13$); \emph{independently}, draw
...
\[
  H_\infty(\mathbf{k}) = \log_2\!\left(\frac{|\mathcal{P}|!}{(|\mathcal{P}|-K)!}\right) \approx K\log_2|\mathcal{P}| \approx 19.7670\,K \text{ bits},
\]
so $H_\infty(\mathbf{k}) = 256.9711$ bits at the default $K=13$
(key-space size $\approx 2.27\times10^{77}\approx 2^{256.97}$), i.e.\
$2^{128.4855}$ against a Grover search. All figures
here are computed from the exact $|\mathcal{P}|=892{,}206$, not from an
approximation to it.

\begin{remark}[Choice of the ``PQ-128'' parameter set]\label{rem:pq128}
The default $(\mathcal{P},K)$ is the smallest parameter set in this family
reaching the $128$-bit post-quantum target. Raising $K$ alone is not
sufficient: over the earlier interval $[10^6,9.9\times10^6]$
($|\mathcal{P}|=579{,}947$, $19.1456$ bits per element) one has
$H_\infty=248.89$ bits at $K=13$, i.e.\ only $124.45$ bits post-Grover.
Widening the interval to $[10^6,1.5\times10^7)$ raises the per-element
contribution to $19.7670$ bits and clears the target at the same $K=13$,
while simultaneously unifying $\mathcal{P}$ with the interval already used
by the key-establishment layer. Since $K$ affects neither the wire format
nor the ciphertext length (Proposition~\ref{prop:expansion-neutral}), the
only cost is stored key material, which grows from $5K+32=82$ to $97$
bytes. Validation deliberately retains no upper bound on key elements
beyond the $2^{40}$ serialisation limit, so keys provisioned under the
earlier interval remain usable.
\end{remark}
...
(Remark~\ref{rem:coarsening-not-inflation}). $K=13$ is an interoperability
...
agree on $\mathbf{k}$ itself.\footnote{CVF-36: an earlier revision of this
remark stated that ``the reference implementations emit a warning below
$K=7$''. No delivered port implements such a warning, and the threshold was
inherited from the earlier single-secret schedule (where
$H_\infty(\mathbf{k})$ entered the bound); the sentence has been removed as
Remark~\ref{rem:key-roles} already establishes that $K$ carries no
security requirement in the v8 construction.} What the arithmetic layer is retained
...
\paragraph{Per-wire-format subkey (domain $\texttt{0x0B}$).}
Before any per-message derivation, a fixed \emph{format subkey}
$\mathsf{sk}_{\mathrm{fmt}}$ is derived from $\mathsf{sk}$ under domain
$\texttt{0x0B}$:
\begin{definition}[Format subkey]\label{def:fmtkey}
\[
  \mathsf{sk}_{\mathrm{fmt}}
    \;=\;
  \mathrm{HMAC}\!\left(\mathsf{sk},\ \texttt{0x0B}\,\|\,\mathrm{format\_id}\right)
    \;\in\;\{0,1\}^{256},
\]
where $\mathrm{format\_id}\in\{0,1\}^{8}$ is a 1-byte constant
identifying the wire format. This paper specifies exactly one format,
$\mathrm{format\_id}=\texttt{0x01}$ (block mode). The identifier
$\mathrm{format\_id}=\texttt{0x02}$ is reserved for a streaming-AE
format that is outside the scope of this paper
(Section~\ref{sec:caveats}). Domain $\texttt{0x0B}$ takes no nonce and
no context beyond the 1-byte identifier; the derivation is invoked once
per key install, not per message.
\end{definition}
The purpose of this layer is \emph{cross-format tag separation}: because
every subsequent HMAC call is keyed by $\mathsf{sk}_{\mathrm{fmt}}$
rather than $\mathsf{sk}$, a tag or ciphertext produced under one
$\mathrm{format\_id}$ cannot verify under another sharing the same
$(\mathbf{k},\mathsf{sk})$. The ciphertext itself carries no
$\mathrm{format\_id}$ byte on the wire — format selection is an
out-of-band API contract — so a ciphertext produced under one format
subkey simply fails the tag comparison at the receiver and is
indistinguishable from a wrong-key ciphertext.

\paragraph{Per-message derivations.}
All \emph{other} internal values are computed as
\[
  \mathrm{Derive}_d(\mathsf{sk}_{\mathrm{fmt}}, N, \mathsf{ctx})
    \;=\;
  \mathrm{HMAC}\!\left(\mathsf{sk}_{\mathrm{fmt}},\ d\,\|\,N\,\|\,\mathsf{ctx}\right)
  \qquad (d \neq \texttt{0x0B}),
\]
...
for AAD $A$). Domain $\texttt{0x0A}$ is the single further exception: it
does not take a nonce as input, since it is the derivation that
\emph{produces} the nonce (Definition~\ref{def:synthnonce}); its input
shape is instead $\texttt{0x0A}\|\mathrm{be8}(|A|)\|A\|M$.
...
\texttt{0x0B} & Format subkey & $\mathrm{format\_id}$ (1 byte, no nonce) & 32 bytes \\
\bottomrule
\end{tabular}
\caption{Domain-separated HMAC derivation schedule. Domain
$\texttt{0x0B}$ is keyed by $\mathsf{sk}$ and produces the format subkey
$\mathsf{sk}_{\mathrm{fmt}}$ (Definition~\ref{def:fmtkey}); every other
domain is keyed by $\mathsf{sk}_{\mathrm{fmt}}$, not by $\mathsf{sk}$
directly. The nonce $N$ is applied uniformly by $\mathrm{Derive}_d$ for
$d\notin\{\texttt{0x0A},\texttt{0x0B}\}$ and is therefore omitted from
the Context column above.}
\label{tab:domains}
\end{table}

\begin{lemma}[Domain separation]\label{lem:domsep}
Write $\mathsf{enc}(d,N,\mathsf{ctx})$ for the HMAC-SHA256 input byte
string that $\mathrm{Derive}_d(\cdot,N,\mathsf{ctx})$ keys HMAC over,
laid out as in Table~\ref{tab:domains} (keyed by $\mathsf{sk}$ for
$d=\texttt{0x0B}$ and by $\mathsf{sk}_{\mathrm{fmt}}$ for every other
$d$; the key does not enter the encoding argument itself). Then
$\mathsf{enc}$ is injective on its \emph{entire} argument domain: for any
two argument tuples
$(d_1,N_1,\mathsf{ctx}_1)\neq(d_2,N_2,\mathsf{ctx}_2)$ with each field
drawn from its declared range, the two resulting HMAC-SHA256 inputs are
distinct. The quantification is over all well-formed argument tuples,
\emph{not} merely over those this construction would itself generate; in
particular it covers tuples chosen adversarially, as
Theorem~\ref{thm:int-ctxt} requires. The statement is unconditional — it
is a property of the encoding alone, independent of $\mathsf{sk}$,
$\mathsf{sk}_{\mathrm{fmt}}$, and of any computational assumption.
\end{lemma}
...
is unconditional. Fix a domain $d\notin\{\texttt{0x0A},\texttt{0x0B}\}$:
bytes $1..16$ carry the nonce, so if $N_1\neq N_2$ the two inputs already
differ there; assume therefore $N_1=N_2$, which reduces distinctness to
injectivity of the context encoding. For domains
...
$(A,M)$. Domain $\texttt{0x0B}$ has the shortest input of any tabulated
domain — the two bytes $\texttt{0x0B}\|\mathrm{format\_id}$ — and hence
cannot collide with any other domain's input, which is at least nine
bytes long ($\texttt{0x0A}\|\mathrm{be8}(|A|)$ even for $A=\varepsilon$)
or seventeen bytes long (all nonce-carrying domains); within
$\texttt{0x0B}$ itself, distinct 1-byte format identifiers give distinct
inputs at byte position $1$. Every case is a statement about byte layout
that holds for arbitrary field values, so $\mathsf{enc}$ is injective on
the whole domain: any two distinct argument tuples, whatever their
provenance, map to distinct HMAC-SHA256 inputs.
\end{proof}

\begin{remark}[Audit finding CV-11: undocumented $\texttt{0x0B}$
format-subkey layer]\label{rem:cv11}
Previous revisions of this section stated the schedule as
$\mathrm{Derive}_d(\mathsf{sk},N,\mathsf{ctx})=\mathrm{HMAC}(\mathsf{sk},d\|N\|\mathsf{ctx})$
without introducing the $\texttt{0x0B}$ format-subkey layer, and
Table~\ref{tab:domains}'s caption asserted ``every domain is keyed by
$\mathsf{sk}$ only''. The reference implementations
(\texttt{napqes.py::\_derive\_format\_subkey},
\texttt{C/napqes.c::derive\_format\_subkey},
\texttt{rust/src/lib.rs::derive\_format\_subkey}), however, thread
$\mathsf{sk}_{\mathrm{fmt}}$ through every per-message derivation rather
than $\mathsf{sk}$, and the shipped KAT corpus
(\texttt{tests/kat/v8\_vectors.json}) reproduces byte-for-byte only
under $\mathsf{sk}\mapsto\mathsf{sk}_{\mathrm{fmt}}$: an independent
implementation written from the earlier text produces \emph{none} of the
twelve positive vectors. The defect was in the specification, not the
port; the extra PRF layer is deliberate and buys cross-format tag
separation, so this section, Table~\ref{tab:domains}, Lemma~\ref{lem:domsep},
the four security theorems of Section~\ref{sec:security}, and the
Section~\ref{sec:caveats} ``Scope of $\mathsf{sk}$ and $\mathbf{k}$
compromise'' caveat have all been restated in terms of
$\mathsf{sk}_{\mathrm{fmt}}$ as it is actually computed. Each theorem's
bound gains one
$\mathrm{Adv}^{\mathrm{PRF}}_{\mathrm{HMAC\text{-}SHA256}}(\mathcal{B}_0)$
term for the single-query $\texttt{0x0B}$ hop — a quantitatively
negligible correction, since the extra HMAC call is against a uniform
256-bit $\mathsf{sk}$, but a necessary bookkeeping change so that the
object proved matches the object computed.
\end{remark}

\begin{remark}[Worked example: format subkey and synthetic nonce for
KAT vector \texttt{W001}]\label{rem:fmtkey-example}
So that an independent implementer can localise a divergence in the
first sixteen output bytes rather than only as a whole-ciphertext
disagreement, we give the two derivations that key everything else,
computed from the paper's text alone against KAT vector \texttt{W001}
of \texttt{tests/kat/v8\_vectors.json} (empty message, empty AAD,
$\mathsf{sk}=$ \texttt{2f89bc687a7987f421d041bedb1c371f8ef741991f343e57b84f4b0c7875fa86},
$\mathrm{format\_id}=\texttt{0x01}$):
\begin{align*}
  \mathsf{sk}_{\mathrm{fmt}}
    &= \mathrm{HMAC}(\mathsf{sk},\ \texttt{0x0B}\|\texttt{0x01}) \\
    &= \texttt{c54c0012905b06e2a36e3242d443267ddd39abf3ef7b16d5592ee6e6e319dc6d}, \\[0.5ex]
  N
    &= \mathrm{HMAC}(\mathsf{sk}_{\mathrm{fmt}},\ \texttt{0x0A}\|\mathrm{be8}(0))[0{:}16] \\
    &= \texttt{e5bb595068e24fd5630555256b5d3446}.
\end{align*}
The 16-byte $N$ matches the first sixteen bytes of \texttt{W001}'s
$\mathrm{ciphertext\_hex}$ (Section~\ref{sec:wire-format}: $C=N\|B\|T$).
An implementation that keyed the schedule by $\mathsf{sk}$ directly would
produce a different sixteen-byte prefix, letting the mismatch be
diagnosed without a byte-by-byte diff of the full ciphertext.
\end{remark}
...
arithmetic; no floating-point representation is used anywhere in the v8
construction $\Pi$ specified by this paper.\footnote{CVF-30: the legacy v7
wire format retained in the reference implementations for backward
compatibility with archived ciphertexts uses an IEEE-754 f64 form of the
noise threshold. v7 encryptors are \texttt{\#[deprecated]} per CVF-17; v7
decryptors are kept so archived ciphertexts remain decodable. The v7 f64
path is documented in \texttt{docs/CAVEATS.md} under CVF-30.} Fix the public integer constants
\[
  \theta_{\min} = \lfloor 0.75 \times 2^{64} \rfloor, \qquad
  \theta_{\max} = \lfloor 0.99 \times 2^{64} \rfloor,
\]
and let $\tau = \mathrm{Derive}_{0x02}(\mathsf{sk}_{\mathrm{fmt}}, N, \varepsilon)[0:8]_{\mathrm{be64}} \in [0, 2^{64})$
...
\[
  \mathbf{pad}_{i} = \bigl(\mathrm{Derive}_{0x06}(\mathsf{sk}_{\mathrm{fmt}}, N, \mathrm{be4}(i))[0{:}4]_{\mathrm{be32}} \bmod 95\bigr) + 32 \ \in [32,126].
\]
...
\begin{align*}
  \texttt{is\_noise}(ct\_pos) &= \mathbf{1}\bigl[\,\mathrm{Derive}_{0x00}(\mathsf{sk}_{\mathrm{fmt}}, N, \mathrm{be5}(ct\_pos))[0{:}8]_{\mathrm{be64}} < \theta(N)\,\bigr], \\
  \texttt{noise\_char}(ct\_pos) &= \bigl(\mathrm{Derive}_{0x04}(\mathsf{sk}_{\mathrm{fmt}}, N, \mathrm{be5}(ct\_pos))[0{:}4]_{\mathrm{be32}} \bmod 96\bigr) + 32, \\
  \texttt{noise\_addend}(ct\_pos, k) &= \bigl(\mathrm{Derive}_{0x05}(\mathsf{sk}_{\mathrm{fmt}}, N, \mathrm{be5}(ct\_pos))[0{:}4]_{\mathrm{be32}} \bmod (k-1)\bigr) + 1, \\
  \texttt{real\_addend}(real\_idx, k) &= \bigl(\mathrm{Derive}_{0x01}(\mathsf{sk}_{\mathrm{fmt}}, N, \mathrm{be5}(real\_idx))[0{:}4]_{\mathrm{be32}} \bmod (k-1)\bigr) + 1.
\end{align*}
\end{definition}
...
For $\mathsf{sk}_{\mathrm{fmt}}\in\{0,1\}^{256}$ (the format subkey of
Definition~\ref{def:fmtkey}), $A\in\{0,1\}^{*}$, $M\in\mathcal{M}$
(encoded as UTF-8 bytes),
\[
  N(\mathsf{sk}_{\mathrm{fmt}}, A, M) \;=\; \mathrm{HMAC}\bigl(\mathsf{sk}_{\mathrm{fmt}},\;
...
\[
  ...
  \qquad
  \mathrm{ks}(N) \;=\; \mathrm{Derive}_{0x07}(\mathsf{sk}_{\mathrm{fmt}}, N, \mathrm{be4}(0)) \,\|\,
  \mathrm{Derive}_{0x07}(\mathsf{sk}_{\mathrm{fmt}}, N, \mathrm{be4}(1)) \,\|\, \cdots,
\]
generating only as many 32-byte blocks as needed to cover $|B|$ bytes
(truncating the final block). The authentication tag is
\[
  T = \mathrm{Derive}_{0x03}\bigl(\mathsf{sk}_{\mathrm{fmt}},\ N,\ \mathrm{TagIn}(A,\widetilde{B})\bigr),
\]
...
(0) derive $\mathsf{sk}_{\mathrm{fmt}}$
(Definition~\ref{def:fmtkey}, once per key install rather than per
message); (1) compute $N=N(\mathsf{sk}_{\mathrm{fmt}},A,M)$
(Definition~\ref{def:synthnonce}); (2) pad $M$ to $P_d$
(Section~\ref{sec:padding}); (3) run the token-emission loop of
Section~\ref{sec:token-construction} over $P_d$ using
$(\mathsf{sk}_{\mathrm{fmt}},N)$ to obtain the token vector
$(\mathrm{token}[0],\ldots,\mathrm{token}[N_{\mathrm{tok}}-1])$; (4)
serialise $B$; (5) mask $\widetilde{B}=B\oplus\mathrm{ks}(N)$; (6) compute
$T=\mathrm{Derive}_{0x03}(\mathsf{sk}_{\mathrm{fmt}},N,\mathrm{TagIn}(A,\widetilde{B}))$
...
using the same $(\mathsf{sk}_{\mathrm{fmt}},N)$-derived noise-position oracle: skip at most
$\mathrm{MAX\_NOISE\_RUN}$ consecutive noise positions, then read one real
token $t$; if $t<a$ or $(t-a)\not\equiv 0\pmod{k}$ output $\bot$
(CVF-27: totality preconditions on the recovery); otherwise recover the
padded codepoint as
$(t-a)/k_{(\mathrm{real\_idx}\bmod K)+1}$; if the recovered value exceeds
$\mathtt{0x10FFFF}$ or is a Unicode surrogate output $\bot$
(also CVF-27, matching the codepoint-range check the reference
implementations perform), where $a$ is the
$\mathrm{real\_idx}$-th addend
$\mathrm{Derive}_{0x01}(\mathsf{sk}_{\mathrm{fmt}},N,\mathrm{real\_idx})$ and
...
time such that
$\Pr[N(\mathsf{sk}_{\mathrm{fmt}},A_1,M_1)=N(\mathsf{sk}_{\mathrm{fmt}},A_2,M_2)] \le \mathrm{Adv}^{\mathrm{PRF}}_{\mathrm{HMAC\text{-}SHA256}}(\mathcal{B}) + 2^{-128}$.
\end{lemma}

\begin{proof}
By Lemma~\ref{lem:domsep}, $(A_1,M_1)\neq(A_2,M_2)$ implies the two domain
$\texttt{0x0A}$ inputs are distinct, so replacing $\mathrm{HMAC}(\mathsf{sk}_{\mathrm{fmt}},\cdot)$
...
encryption queries, there exist PRF adversaries
$\mathcal{B}_0,\mathcal{B}_1$ with similar running time such that
\[
  \mathrm{Adv}^{\mathrm{IND\text{-}CPA\text{-}det}}_{\Pi}(\mathcal{A})
  \;\leq\;
  \mathrm{Adv}^{\mathrm{PRF}}_{\mathrm{HMAC\text{-}SHA256}}(\mathcal{B}_0)
...
key-guessing or key-entropy term. The $\mathcal{B}_0$ term accounts for
the one-query PRF hop that replaces
$\mathsf{sk}_{\mathrm{fmt}}=\mathrm{HMAC}(\mathsf{sk},\texttt{0x0B}\|\mathrm{format\_id})$
(Definition~\ref{def:fmtkey}) by a uniform 256-bit string; the
$\mathcal{B}_1$ term is the subsequent PRF hop against
$\mathrm{HMAC}(\mathsf{sk}_{\mathrm{fmt}},\cdot)$, as in the proof below.
\end{theorem}

\begin{proof}
We proceed via a sequence of three hybrid games.

\paragraph{Game $G_0$ (real).} The challenger samples $(\mathbf{k},\mathsf{sk})$,
computes
$\mathsf{sk}_{\mathrm{fmt}}=\mathrm{HMAC}(\mathsf{sk},\texttt{0x0B}\|\mathrm{format\_id})$
once (Definition~\ref{def:fmtkey}), and answers queries via the real
$\mathsf{Enc}$, which calls $F(\mathsf{sk}_{\mathrm{fmt}},\cdot)$ for all
per-message derivations.

\paragraph{Game $G_0'$ (format-subkey PRF hop).} Identical to $G_0$,
except $\mathsf{sk}_{\mathrm{fmt}}$ is sampled uniformly from
$\{0,1\}^{256}$ rather than derived by
$\mathrm{HMAC}(\mathsf{sk},\texttt{0x0B}\|\mathrm{format\_id})$. Because
$\mathsf{sk}$ is uniform on $\{0,1\}^{256}$
(Definition~\ref{def:keygen}) and $\texttt{0x0B}\|\mathrm{format\_id}$ is
a single fixed input, replacing $\mathrm{HMAC}(\mathsf{sk},\cdot)$ by a
truly random function on this one query is a PRF hop of advantage at
most $\mathrm{Adv}^{\mathrm{PRF}}_{\mathrm{HMAC\text{-}SHA256}}(\mathcal{B}_0)$
for some PRF adversary $\mathcal{B}_0$ running in time
$\mathcal{O}(T_\mathcal{A})$; the output of that single call is
distributed identically to a uniformly sampled 256-bit string, so
$|\Pr[\mathcal{A}\text{ wins }G_0] - \Pr[\mathcal{A}\text{ wins }G_0']| \le \mathrm{Adv}^{\mathrm{PRF}}_{\mathrm{HMAC\text{-}SHA256}}(\mathcal{B}_0)$.

\paragraph{Game $G_1$ (per-message PRF replacement).} Identical to
$G_0'$, except $F(\mathsf{sk}_{\mathrm{fmt}},\cdot)$ is replaced by a
truly random function $\rho:\{0,1\}^*\to\{0,1\}^{256}$, sampled lazily.
...
$F(\mathsf{sk}_{\mathrm{fmt}},\cdot)$ (for a uniform 256-bit
$\mathsf{sk}_{\mathrm{fmt}}$) or $\rho(\cdot)$. Because
$\mathsf{sk}_{\mathrm{fmt}}$ is uniform on $\{0,1\}^{256}$ in $G_0'$ and
is sampled independently of $\mathbf{k}$ (Definition~\ref{def:keygen}
plus the format-subkey hop above), $\mathcal{B}_1$ samples its own
$\mathbf{k}'$ via the prime-sampling step of $\mathsf{KeyGen}$ to run
the arithmetic layer of $\mathsf{Enc}$ directly, and forwards every
per-message domain-derivation call (including domain $\texttt{0x0A}$) to
$\mathcal{O}$. $\mathcal{B}_1$ never needs to know
$\mathsf{sk}_{\mathrm{fmt}}$ (or the challenger's hidden key) to run
this simulation: when $\mathcal{O}=F(\mathsf{sk}_{\mathrm{fmt}},\cdot)$
the simulation is distributed identically to $G_0'$; when
$\mathcal{O}=\rho$, identically to $G_1$.
\end{proof}
...
\[
  ...
  \le \mathrm{Adv}^{\mathrm{PRF}}_{\mathrm{HMAC\text{-}SHA256}}(\mathcal{B}_0)
    + \mathrm{Adv}^{\mathrm{PRF}}_{\mathrm{HMAC\text{-}SHA256}}(\mathcal{B}_1)
    + \frac{q^2}{2^{128}}, \qedhere
\]
where the $\mathcal{B}_0$ term is the $G_0\to G_0'$ hop above and the
$\mathcal{B}_1$ term is the $G_0'\to G_1$ hop of
Lemma~\ref{lem:prf-hop}.
\end{proof}
...
term for the relaxed length condition. The $\mathcal{B}_0$ hop is the
same single-query $\texttt{0x0B}$ format-subkey hop as in
...
and at most $q_v\ge1$ adaptive verification queries. There exist PRF
adversaries $\mathcal{B}_0,\mathcal{B}_2$ with similar running time such
that
\[
  \mathrm{Adv}^{\mathrm{INT\text{-}CTXT}}_{\Pi}(\mathcal{A})
  \leq
...
below is resolved deterministically rather than probabilistically. The
$\mathcal{B}_0$ term accounts for the one-query PRF hop that replaces
$\mathsf{sk}_{\mathrm{fmt}}=\mathrm{HMAC}(\mathsf{sk},\texttt{0x0B}\|\mathrm{format\_id})$
by a uniform 256-bit string (Definition~\ref{def:fmtkey}); the
$\mathcal{B}_2$ term is the subsequent PRF hop against
$\mathrm{HMAC}(\mathsf{sk}_{\mathrm{fmt}},\cdot)$ used in the proof
below.
\end{theorem}

\begin{proof}
As in Theorem~\ref{thm:ind-cpa}, we first apply the format-subkey PRF
hop: replace
$\mathsf{sk}_{\mathrm{fmt}}=\mathrm{HMAC}(\mathsf{sk},\texttt{0x0B}\|\mathrm{format\_id})$
by an independent, uniform 256-bit string, at cost
$\mathrm{Adv}^{\mathrm{PRF}}_{\mathrm{HMAC\text{-}SHA256}}(\mathcal{B}_0)$
for some PRF adversary $\mathcal{B}_0$. Under the modified game,
$\mathsf{sk}_{\mathrm{fmt}}$ is now a uniform HMAC key independent of
$\mathsf{sk}$ and $\mathbf{k}$, and every subsequent argument proceeds
against this key.
...
$\mathrm{HMAC}(\mathsf{sk}_{\mathrm{fmt}},\cdot)$. Construct $\mathcal{B}_2$, given an
...
Combining both cases with the initial format-subkey PRF hop yields the
stated bound.
\end{proof}
...
PRF adversaries $\mathcal{B}_0,\mathcal{B}_1,\mathcal{B}_2$ with similar
...
\[
  ...
  + \frac{q_d}{2^{256}}.
\]
The $\mathcal{B}_0$ term is the single-query PRF hop replacing
$\mathsf{sk}_{\mathrm{fmt}}=\mathrm{HMAC}(\mathsf{sk},\texttt{0x0B}\|\mathrm{format\_id})$
(Definition~\ref{def:fmtkey}) by a uniform 256-bit string, and is
inherited from Theorems~\ref{thm:ind-cpa} and~\ref{thm:int-ctxt}. Once
$\mathsf{sk}_{\mathrm{fmt}}$ is uniform, $\mathcal{B}_1$ and
$\mathcal{B}_2$ are the same downstream PRF hops as in those two
theorems (a single $\mathcal{B}_0$ term suffices, since Theorems
$\ref{thm:ind-cpa}$ and $\ref{thm:int-ctxt}$'s $\mathcal{B}_0$ hops
target the identical $\mathsf{sk}$-keyed HMAC call).
\end{theorem}
...
Before the game hops below, apply the format-subkey PRF hop of
Theorem~\ref{thm:ind-cpa}'s proof once, replacing
$\mathsf{sk}_{\mathrm{fmt}}=\mathrm{HMAC}(\mathsf{sk},\texttt{0x0B}\|\mathrm{format\_id})$
by an independent uniform 256-bit string at cost
$\mathrm{Adv}^{\mathrm{PRF}}_{\mathrm{HMAC\text{-}SHA256}}(\mathcal{B}_0)$.
Because both downstream reductions (to $\mathrm{Adv}^{\mathrm{PRF}}(\mathcal{B}_1)$
via Theorem~\ref{thm:ind-cpa} and to
$\mathrm{Adv}^{\mathrm{PRF}}(\mathcal{B}_2)$ via
Theorem~\ref{thm:int-ctxt}) invoke exactly the same format-subkey hop
against the same $\mathsf{sk}$-keyed HMAC call, one $\mathcal{B}_0$ term
suffices for the composition rather than two.
...
\begin{itemize}
  ...
  \item $\mathbf{k}$: $\approx2^{256.97}$ ordered-tuple key-space size at the
        default $K=13$ (post-Grover $\approx2^{128.49}$). This figure is
...
Key size & $5K+32$ bytes ($\mathbf{k}$+$\mathsf{sk}$; $97$ at $K=13$) & 32 bytes & 32 bytes & 16 bytes \\
\bottomrule
\end{tabular}
...
\begin{description}
  ...
  \item[Scope of $\mathsf{sk}$ and $\mathbf{k}$ compromise.] $\mathsf{sk}$
        is the sole cryptographic secret in this construction: one HMAC
        call keyed by $\mathsf{sk}$ (the domain-$\texttt{0x0B}$
        format-subkey derivation of Definition~\ref{def:fmtkey}) fixes
        $\mathsf{sk}_{\mathrm{fmt}}$, and every other HMAC domain
        derivation — the noise schedule, the padding, the per-position
        addends, the keystream, and the tag — is keyed by
        $\mathsf{sk}_{\mathrm{fmt}}$ (Section~\ref{sec:domains}).
        Consequently, an adversary who compromises $\mathsf{sk}$ but not
        $\mathbf{k}$ derives $\mathsf{sk}_{\mathrm{fmt}}$ in one HMAC
        evaluation and can then recover any real token's plaintext
        codepoint directly: the per-position addend's raw HMAC output
        depends only on $\mathsf{sk}_{\mathrm{fmt}}$, so for a given
        token the adversary can, for each of the $892{,}206$ candidate
        primes in $\mathcal{P}$, compute the corresponding addend and
        check whether the resulting quotient is an exact integer in a
        valid codepoint range — an $\mathcal{O}(|\mathcal{P}|)$
        trial-division search that succeeds in microseconds.
        $\mathbf{k}$'s secrecy therefore adds \textbf{no} independent
        confidentiality margin once $\mathsf{sk}$ is known: compromise of
        $\mathsf{sk}$ alone is equivalent to a full break, regardless of
        whether $\mathbf{k}$ also leaked. Operationally,
