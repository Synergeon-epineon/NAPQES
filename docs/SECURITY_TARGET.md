...
| Traffic analysis: per-message length correlation with content | Partially (bounded) | Ciphertext byte-length is a deterministic function of the **padding bucket alone** — independent of codepoint values, of the prime key `k`, and of `K` (`lem:length-det`). The scheme therefore meets LH-IND-CPA-det, a strengthening of IND-CPA-det in which the two challenge plaintexts need only share a bucket rather than a length (Theorem `thm:lh-ind-cpa`, `docs/napseq-eprint-v3.tex`), at the same bound. Residual leakage is at most `log₂13 ≈ 3.70` bits per message under the default `bucket` profile and exactly **zero** under the `frame(F)` profile (Corollary `cor:length-leak`). Note this is an average-case bound: a specific two-class distinguisher can still succeed under `bucket` — see `traffic_analysis_bench.py`. |
