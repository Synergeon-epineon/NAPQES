...
separator prepended to the HMAC message, followed immediately by the
nonce, to ensure derivation functions are computationally independent.

> **CVF2 fix.** Prior revisions of this draft placed the domain separator
> *after* the nonce for domains `0x00`–`0x02`, `0x04`–`0x07`, while the
> authentication tag (`0x03`) placed the nonce *after* the AAD. This
> inconsistency between the domain table below, the security proof, and
> the reference code was audit finding CVF2. All domains now use the same
> layout, `HMAC(key_bytes, domain_byte || nonce || ctx)`, making domain
> separation an unconditional (not probabilistic) property: every domain
> differs at byte 0, and the nonce occupies the same fixed offset in every
> domain.
...
~~~
d   = HMAC(key_bytes, 0x00 || nonce || be5(ct_pos))
...
~~~
d      = HMAC(key_bytes, 0x01 || nonce || be5(real_idx))
...
~~~
d_c        = HMAC(key_bytes, 0x04 || nonce || be5(ct_pos))
noise_c    = (bytes_to_uint32_big(d_c[0:4]) mod 96) + 32  ; in [32, 127]

d_a        = HMAC(key_bytes, 0x05 || nonce || be5(ct_pos))
...
~~~
d      = HMAC(key_bytes, 0x06 || nonce || be4(i))
...
**CVF1 fix (v7).** Each token is encoded as a fixed-width, big-endian
8-byte unsigned integer field.  The encoded tokens are concatenated to form
`token_blob`, so `len(token_blob) == 8 * token_count`.  Because every token
occupies the same width regardless of magnitude, `token_blob`'s length
depends only on `token_count` (itself a function of the padded codepoint
count and the HMAC-derived, content-independent noise schedule), never on
the magnitude of individual tokens.

Prior to this fix (retired v6 format), each token was encoded as an
unsigned LEB128 (base-128) varint (7 bits of value per byte, MSB = 1 if
more bytes follow, MSB = 0 on the final byte).  Because `token = codepoint *
key_element + addend`, the LEB128 byte-length of a token grew with the
plaintext codepoint, so `varint_blob`'s length depended on plaintext
content even between messages of equal padded length — breaking the
IND-CPA hiding argument (Section 8.1, "Confidentiality (IND-CPA)").  See
audit finding CVF1 in the reference implementation's `docs/CAVEATS.md` for
the full distinguisher and proof erratum.  Decoders MAY still support the
legacy LEB128 format for reading pre-fix ciphertexts (opt-in only); new
ciphertexts MUST use the fixed-width encoding.

# Wire Format — Version 7 (supersedes Version 6, CVF1 fix)

The binary wire format for block-mode encryption is:

~~~
ciphertext = nonce(16) || masked_blob(token_count * 8) || auth_tag(32)
~~~

- `nonce` (16 bytes): uniformly random, generated by a FIPS 140-3 compliant
  DRBG per encryption call.
- `masked_blob` (`token_count * 8` bytes): the domain-0x07 XOR-masked,
  fixed-width token sequence as described in Section 5.1.
...
tag_msg  = 0x03 || nonce || be4(aad_len) || aad || masked_blob
auth_tag = HMAC(key_bytes, tag_msg)    ; 32 bytes
~~~

Note (CVF2 fix): the nonce now occupies the fixed byte 1..16 offset shared
with every other domain in the derivation schedule (Section "Domain-
Separated HMAC Derivation"); it was previously placed after the AAD.
...
4. Unmask `masked_blob` and decode the fixed-width token sequence (or, for
   legacy v6 ciphertexts under explicit opt-in, the LEB128 sequence), run
   the decrypt loop, unpad, return plaintext.
...
- **Confidentiality (IND-CPA):** Under the PRF assumption on HMAC-SHA256,
  ...
  **This claim requires the fixed-width (v7) token encoding of Section 5.1**:
  under the retired v6 LEB128 encoding, ciphertext byte-length was a function
  of plaintext codepoint values (not just padded length), which by itself
  gave a distinguisher with IND-CPA advantage ≈ 1 regardless of the PRF
  assumption (audit finding CVF1). The full proof, including the erratum
  for the flawed v6 argument, is in `docs/napseq-eprint-preprint.tex`
  Theorem 1 / Section "Wire Format (Version 7 — CVF1 fix)".
...
As of the v7 fixed-width token encoding (CVF1 fix), ciphertext length within
a given padding bucket no longer depends on plaintext codepoint *values* —
only the bucket itself (CAV-003) is disclosed. Implementations using the
retired v6 LEB128 token encoding leaked additional information: two
plaintexts in the same padding bucket could still produce ciphertexts of
different byte-length depending on codepoint magnitude.
