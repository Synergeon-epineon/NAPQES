# NAPSEQ Approved-Primitives Pre-Attestation
...
> NAPSEQ Python reference (`napqes.py`) and maps each to the applicable NIST
> standard. It supports a pre-attestation claim that NAPSEQ uses only
> FIPS-approved sub-primitives in approved modes, **without** asserting that
> the NAPSEQ module as a whole is FIPS 140-3 validated.
>
> **Non-claim.** NAPSEQ is NOT FIPS 140-3 validated. This memo documents the
...
The following primitives are **absent** from the NAPSEQ v6 reference
implementation. This is stated to prevent incorrect assumptions:

| Primitive | Status | Note |
|---|---|---|
| AES (any mode) | **Not used** | By design; NAPSEQ targets non-AES-hardware environments |
...
> NAPSEQ v6 uses FIPS-approved cryptographic sub-primitives: HMAC-SHA256
...
> and key generation.
>
> **The NAPSEQ module itself is not FIPS 140-3 validated.** FIPS 140-3
...
> NAPSEQ as a supplementary layer.
...
|---|---|
| "NAPSEQ is not a NIST-standardised cipher" | §3 FIPS Posture Statement |
| "NAPSEQ is not a post-quantum KEM or signature" | §2 Primitives NOT Used |
...
- [ ] Confirm FIPS 198-1 key-length requirement is met for minimum key size
