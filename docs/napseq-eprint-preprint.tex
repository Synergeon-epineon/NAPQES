...
\begin{description}
  ...
  \item[CAV-002 — 16-bit length cap.] The 2-codepoint big-endian length prefix
        in the padding scheme limits block-mode plaintext to 65\,535 codepoints.
        Raising this cap requires a v7 wire format (deferred).
  \item[CAV-003 — Padding length-bucket leak.] Ciphertext length reveals the
        power-of-two padding bucket.  Callers needing full length-hiding must
        apply a fixed-frame transport.
  \item[CAV-004 — Ciphertext expansion.] Noise injection produces $4\text{--}20\times$
        expansion, traded against traffic-analysis resistance.
  \item[CAV-005 — Legacy unauthenticated path.] The \texttt{allow\_legacy\_unauthenticated}
        flag re-enables decoding of v2/v3/v5 formats without authentication.
        Callers should not expose this flag to untrusted inputs.
\end{description}
