...
$\operatorname{NAPSEQ}.\mathsf{Enc}$ algorithm (which calls
...
$c^*\gets\operatorname{NAPSEQ}.\mathsf{Enc}(\mathbf{k},m_b)$, and
...
Each $\rho$-block is fresh and uniform, and domain~\texttt{0x07} is disjoint
from domains \texttt{0x00}--\texttt{0x06} used to construct $B$, so
$\mathrm{ks}(N^*)$ is independent of~$B$.  Therefore the masked blob satisfies:
\[
  \tilde{B}
  = B \oplus \mathrm{ks}(N^*)
  \;\overset{\mathrm{d}}{=}\;
  \mathrm{Uniform}\bigl(\{0,1\}^{|B|}\bigr),
\]
independently of the plaintext bit~$b$ and the view~$V$.

Similarly, the authentication tag
$T=\rho(N^*\|\texttt{0x03}\|\mathrm{be4}(|aad|)\|aad\|N^*\|\tilde{B})$
is evaluated at an input that has not appeared in any previous $\rho$
evaluation (it is prefixed by $N^*$ and uses domain~\texttt{0x03} with a
distinct suffix), so $T$ is a fresh uniform $32$-byte value independent
of $b$ and~$V$.
...
\begin{theorem}[INT-CTXT]
  Under the PRF assumption on HMAC-SHA256, NAPSEQ is INT-CTXT secure.
  ...
  \[
    \mathrm{Adv}^{\mathrm{INT\text{-}CTXT}}_{\mathrm{NAPSEQ}}(\mathcal{A})
    \leq
...
calls) such that $\mathrm{NAPSEQ}.\mathsf{Dec}(\mathbf{k}, c^*, aad^*) \neq \bot$.
...
\begin{theorem}[IND-CCA]
  Under the PRF assumption on HMAC-SHA256, NAPSEQ is IND-CCA secure.
  Specifically, for any PPT adversary $\mathcal{A}$ making at most $q$
  ...
  \[
    \mathrm{Adv}^{\mathrm{IND\text{-}CCA}}_{\mathrm{NAPSEQ}}(\mathcal{A})
    \leq
    \mathrm{Adv}^{\mathrm{PRF}}_{\mathrm{HMAC\text{-}SHA256}}(\mathcal{B}_1)
    +
    \mathrm{Adv}^{\mathrm{PRF}}_{\mathrm{HMAC\text{-}SHA256}}(\mathcal{B}_2)
...
The two games differ only when $\mathcal{A}$ submits a fresh valid
ciphertext $c \neq c^*$ to $\mathcal{D}$ that would be accepted in $H_0$.
Any distinguisher between $H_0$ and $H_1$ therefore yields an INT-CTXT
forger $\mathcal{A}'$ (with $q_d$ forgery attempts):
\[
  ...
  + \frac{q_d^2}{2^{128}},
\]
where the second inequality applies Theorem~2 with $q_d$ queries.
...
\begin{align*}
  ...
  &\leq \bigl|\Pr[H_0] - \Pr[H_1]\bigr|
       + \bigl|\Pr[H_1] - \tfrac{1}{2}\bigr| \\
  &\leq \mathrm{Adv}^{\mathrm{PRF}}_{\mathrm{HMAC\text{-}SHA256}}(\mathcal{B}_2)
       + \frac{q_d^2}{2^{128}}
       + \mathrm{Adv}^{\mathrm{PRF}}_{\mathrm{HMAC\text{-}SHA256}}(\mathcal{B}_1)
       + \frac{q^2}{2^{128}}.
\end{align*}
Since $(q^2 + q_d^2) \leq (q + q_d)^2$, this gives the stated bound.
\qedhere
\end{proof}
